...
//============================================================================= 
SYSTEM_EXTERN volatile unsigned char  xdata wifi_data_process_buf[PROTOCOL_HEAD + WIFI_DATA_PROCESS_LMT];     //串口数据处理缓存//20210215sdk
SYSTEM_EXTERN volatile unsigned char  xdata wifi_uart_rx_buf[PROTOCOL_HEAD + WIFI_UART_RECV_BUF_LMT];         //串口接收缓存//20210215sdk
SYSTEM_EXTERN volatile unsigned char xdata wifi_uart_tx_buf[PROTOCOL_HEAD + WIFIR_UART_SEND_BUF_LMT];        //串口发送缓存//20210215sdk
//
SYSTEM_EXTERN volatile unsigned char idata *rx_buf_in;
SYSTEM_EXTERN volatile unsigned char idata *rx_buf_out;

SYSTEM_EXTERN volatile unsigned char idata stop_update_flag;                                                 //ENABLE:停止一切数据上传  DISABLE:恢复一切数据上传

#ifndef WIFI_CONTROL_SELF_MODE
SYSTEM_EXTERN volatile unsigned char idata reset_wifi_flag;                                                  //重置wifi标志(TRUE:成功/FALSE:失败)
SYSTEM_EXTERN volatile unsigned char idata set_wifimode_flag;                                                //设置WIFI工作模式标志(TRUE:成功/FALSE:失败)
SYSTEM_EXTERN volatile unsigned char idata wifi_work_state;                                                  //wifi模块当前工作状态
#endif
